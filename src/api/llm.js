...
const DEFAULT_POLISH_PROMPT = `你是一個文字編輯助手。請將以下語音轉錄的文字潤飾為自然、專業的書面文字。
...
const TRANSLATE_PROMPT_PREFIX = `你是一個翻譯助手。請將以下語音轉錄的文字，在保留原意的前提下，整理後翻譯成指定語言。

規則：
1. 保持原意不變，不要添加新內容
2. 修正明顯的口語贅詞、語序與辨識小錯誤，但不要改變意思
3. 加入適當的標點符號
4. 最終輸出必須完全使用指定語言
5. 只輸出翻譯後的文字，不要加任何說明`;
...
function buildTranslateLanguageRule(targetLanguage) {
  if (!targetLanguage) return '';
  const label = LANGUAGE_LABELS[targetLanguage] || targetLanguage;
  return `

目標語言：${label}
請確保最終輸出完整使用${label}。若原文不是該語言，請翻譯成${label}後輸出。`;
}

function buildPolishPrompt(items, options = {}) {
  if (options.targetLanguage) {
    return `${TRANSLATE_PROMPT_PREFIX}${buildTranslateLanguageRule(options.targetLanguage)}${buildVocabularyRules(items)}`;
  }
  return `${DEFAULT_POLISH_PROMPT}${buildVocabularyRules(items)}`;
}
